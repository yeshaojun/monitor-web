...
export function useRequest<T>(
  ...
  const removeTrim = (params: Record<string, any>) => {
  ...
  const execute = async () => {
    ...
    context.options.params = removeTrim(context.options.params)
    context.options.data = removeTrim(context.options.data)
  ...
  function until() {
    const stop: Function | null = null
    return new Promise((resolve) => {
      watch([isFinished], ([v1]) => {
        if (v1 === true) {
          stop && (stop as Function)()
          resolve(v1)
        }
      })
    })
